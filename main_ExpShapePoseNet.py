        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            with open(output_proc, 'rb') as csvfile:
                ...
                #     Shape = Shape_Texture[0:99]
                    ...
                    # Shape + Expression + Pose
                    info_dict[image_key] = {'faces':faces, 'Shape_Texture':Shape_Texture, 'Expr':Expr, 'Pose':Pose}
                    # SEP,TEP = utils.projectBackBFM_withEP(model, Shape_Texture, Expr, Pose)
                    # utils.write_ply_textureless(outFile + '_Shape_Expr_Pose.ply', SEP, TEP, faces)
            ...
            with open(output_proc, 'rb') as csvfile:
                ...
                #     Shape = Shape_Texture[0:99]
                    ...
                    for k,v in info_dict.items():
                        if k == image_key:
                            continue
                        print('> Process ' + image_key + ' to '+k)
                        SEP,TEP = utils.projectBackBFM_withEP(model, Shape_Texture, v['Expr'], Pose)
                        utils.write_ply_textureless(outFile + '_'+k+'.ply', SEP, TEP, faces)
